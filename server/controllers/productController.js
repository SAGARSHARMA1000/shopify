...
export const getSingleProduct = async (req, res) => {
  ...
  //   console.log("POPULATED:", product.comboProducts);
    res.json(product);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const addProduct = async (req, res) => {
  ...
  : [];
    ...
    if (req.files?.images) {
      ...
      // ✅ Optimized image URL for fast loading
      imageUrls.push(result.secure_url.replace('/upload/', '/upload/q_auto,f_auto/'));
    }
  }
    ...
    if (req.files?.video) {
      ...
      // ✅ Optimized video URL to prevent buffering
      videoUrl = result.secure_url.replace('/upload/', '/upload/q_auto,f_auto,vc_auto/');
    }
    const product = new Product({
      title,
      price,
...
export const updateProduct = async (req, res) => {
  ...
  // ✅ Optimized video URL to prevent buffering
  product.video = result.secure_url.replace('/upload/', '/upload/q_auto,f_auto,vc_auto/');
}
